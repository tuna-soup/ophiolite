...
const RECENTS_KEY = "ophiolite-harness-project-recents";
...
function HomePage({
  ...
  async function openProject(path: string, create: boolean) {
    ...
    setNotice(create ? { tone: "info", title: "Project created", detail: `Created OphioliteProject at ${project.root}` } : null);
    navigate("/project");
  }

  useEffect(() => {
    if (menuAction === "file.new-project") {
      void chooseFolder("Choose a folder for the new Ophiolite project")
        .then((folder) => {
          if (folder) return openProject(folder, true);
        })
        .finally(() => setMenuAction(null));
    } else if (menuAction === "file.open-project") {
      void chooseFolder("Open an existing Ophiolite project")
        .then((folder) => {
        ...
        <header className="grid gap-6 md:grid-cols-[minmax(0,1fr)_320px]">
          <div className="space-y-4">
            ...
            <h1 className="max-w-4xl text-4xl font-semibold tracking-tight md:text-6xl">
              Open a Ophiolite project and inspect multiple wellbore asset families in one workspace.
            </h1>
            <p className="max-w-3xl text-base text-stone-600 md:text-lg">
              The app now starts from `OphioliteProject`, not a single package. Logs, tops,
        ...
        <section className="grid gap-6 lg:grid-cols-[1fr_1fr]">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Create a `OphioliteProject` root and then import assets into it.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-3">
                <Button data-testid="create-project-button" onClick={() => void chooseFolder("Choose a folder for the new Ophiolite project").then((folder) => { if (folder) return openProject(folder, true); })}>Choose Folder</Button>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-3">
                <Button data-testid="open-project-button" onClick={() => void chooseFolder("Open an existing Ophiolite project").then((folder) => { if (folder) return openProject(folder, false); })}>Choose Folder</Button>
...
function ProjectPage({
  ...
  useEffect(() => {
    if (menuAction === "file.new-project") {
      void run(async () => {
        const folder = await chooseFolder("Choose a folder for the new Ophiolite project");
        if (!folder) return;
      ...
      void run(async () => {
        const folder = await chooseFolder("Open an existing Ophiolite project");
        if (!folder) return;
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(209,165,88,0.18),transparent_28%),linear-gradient(180deg,#fbf7ef_0%,#efe4d0_100%)] px-4 py-4 text-stone-900 md:px-6">
      <div className="mx-auto flex max-w-[1700px] flex-col gap-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="space-y-1">
            <Badge tone="accent">OphioliteProject workspace</Badge>
