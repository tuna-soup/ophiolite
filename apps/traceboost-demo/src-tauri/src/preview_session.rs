...
use traceboost_app::{preview_post_stack_neighborhood_processing_label, preview_processing_label};
...
impl PreviewSessionState {
    ...
    > {
        let mut active = self.active.lock().expect("preview session mutex poisoned");
        let session = ensure_active_session_for_section(
            &mut active,
            &request.store_path,
            &request.section.dataset_id.0,
        )?;
        let (section, reuse) =
            seis_runtime::preview_post_stack_neighborhood_processing_section_view_with_prefix_cache(
                &mut session.session,
                request.section.axis,
                request.section.index,
                &request.pipeline,
            ...
            .map_err(|error| error.to_string())?;
        let processing_label = preview_post_stack_neighborhood_processing_label(&request.pipeline);
        Ok((
            PreviewPostStackNeighborhoodProcessingResponse {
                schema_version: IPC_SCHEMA_VERSION,
                preview: PreviewView {
...
fn ensure_active_session<'a>(
    ...
    ensure_active_session_for_section(active, &request.store_path, &request.section.dataset_id.0)
}

fn ensure_active_session_for_section<'a>(
    active: &'a mut Option<ActivePreviewSession>,
    store_path: &str,
    dataset_id: &str,
) -> Result<&'a mut ActivePreviewSession, String> {
    let needs_new_session = active.as_ref().map_or(true, |session| {
        session.store_path != store_path || session.dataset_id != dataset_id
    });
    if needs_new_session {
        let session = PreviewSectionSession::open(store_path).map_err(|error| error.to_string())?;
        let actual_dataset_id = session.dataset_id().0;
