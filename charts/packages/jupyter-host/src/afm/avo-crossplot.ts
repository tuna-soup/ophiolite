import { mountAvoCrossplotChart } from "../index";
...
export default {
  render({ model, el }: { model: { get(name: string): any; on(name: string, cb: () => void): void; off(name: string, cb: () => void): void; set(name: string, value: unknown): void; save_changes(): void }; el: HTMLElement }) {
    let mounted: ReturnType<typeof mountAvoCrossplotChart> | null = null;
    ...
    const syncSource = () => {
      ...
      try {
        if (mounted) {
          ...
          el.replaceChildren();
          mounted = mountAvoCrossplotChart(el, source, {
            chartId: model.get("chart_id") ?? undefined
          });
        }
        model.set("frontend_error", null);
      } catch (error) {
        const message = error instanceof Error ? error.message : "Unknown AVO crossplot widget error.";
