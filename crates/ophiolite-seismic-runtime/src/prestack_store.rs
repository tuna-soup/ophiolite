...
use ophiolite_seismic::{
    ...
    GatherSampleDomain, GatherSelector, GatherView, GeometryDescriptor,
    GeometryProvenanceSummary, GeometrySummary, ProcessingPipelineSpec, SectionColorMap,
    SectionDisplayDefaults, SectionMetadata, SectionPolarity, SectionRenderMode, SectionUnits,
    SeismicLayout, VolumeDescriptor,
};
...
impl TbgathReader {
    pub fn open(root: impl AsRef<Path>) -> Result<Self, SeismicStoreError> {
        ...
        let manifest = serde_json::from_slice::<TbgathManifest>(&fs::read(root.join(MANIFEST_FILE))?)?;
...
impl PrestackStoreHandle {
    ...
    pub fn read_gather_plane(&self, request: &GatherRequest) -> Result<GatherPlane, SeismicStoreError> {
        ensure_dataset_matches(self, &request.dataset_id.0)?;
        let reader = TbgathReader::open(&self.root)?;
        let (iline_index, xline_index) = resolve_gather_selector(&self.manifest, &request.selector)?;
...
pub fn open_prestack_store(root: impl AsRef<Path>) -> Result<PrestackStoreHandle, SeismicStoreError> {
...
fn resolve_gather_selector(
    ...
    match selector {
        ...
        GatherSelector::Ordinal { index } => {
            ...
            Ok((index / manifest.volume.shape[1], index % manifest.volume.shape[1]))
        }
    }
}
...
mod tests {
                ...
                for offset_index in 0..manifest.gather_count() {
                    for sample_index in 0..manifest.volume.shape[2] {
                        let value = (iline_index as f32 * 1000.0)
                            ...
                            + sample_index as f32;
                        values[gather_start + offset_index * manifest.volume.shape[2] + sample_index] =
                            value;
                    }
                }
            }
    ...
    fn prestack_descriptor_exposes_layout_and_gather_axis() {
        ...
        assert_eq!(descriptor.geometry.summary.gather_axis.as_ref().map(|axis| axis.count), Some(3));
    }
    ...
    fn materialize_gather_processing_store_applies_offset_mute() {
        ...
        let plane = derived.read_gather_plane(&request).expect("derived gather should read");
