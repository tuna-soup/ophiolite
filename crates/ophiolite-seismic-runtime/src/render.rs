...
use ophiolite_seismic::{SectionAxis, SectionRequest};
...
fn write_section_csv(
    ...
    match axis {
        SectionAxis::Inline => {
            for xline in 0..handle.manifest.volume.shape[1] {
                write!(file, "{}", handle.manifest.volume.axes.xlines[xline])?;
                for sample in 0..handle.manifest.volume.shape[2] {
        ...
        SectionAxis::Xline => {
            for iline in 0..handle.manifest.volume.shape[0] {
                write!(file, "{}", handle.manifest.volume.axes.ilines[iline])?;
                for sample in 0..handle.manifest.volume.shape[2] {
